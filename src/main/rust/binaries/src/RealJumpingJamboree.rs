...
fn main() {
    ...
    let cl_method = prepare_static_method(env, "com/rusty/JavaJungleJuice", "calc", "(I)I");
    ...
    let nbJniTransition = number_of_iterations;
    println!("Rust -> Java -> Java : res = {} Total execution time for JNI transitions (R->J) = {} \t\t: {:?}ms", error, nbJniTransition, execution_time.as_millis());
}
...
fn calc(n: i32, env: *mut JNIEnv, class: jclass, method: jmethodID) -> i32 {
    actual_call(env, class, method, n, 0)
}
